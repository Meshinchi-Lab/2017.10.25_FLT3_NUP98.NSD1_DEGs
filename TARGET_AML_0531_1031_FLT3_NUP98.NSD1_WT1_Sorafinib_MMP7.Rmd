---
title: 'TKIs Response in NUP98-NSD1 and WT1 mutants: MMP7'
author: "Jenny Smith"
date: "May 16, 2018"
...
```{r}
MMP7.expn.0531 <- tpm.0531 %>%
  rownames_to_column("Gene") %>%
  filter(grepl("MMP7$", Gene)) %>%
  ...
  left_join(., 
            select(CDE.0531,TARGET.USI.1,FLT3.ITD.positive.,WT1.mutation,NPM.mutation,CEBPA.mutation,NUP98.NSD1,
                   Overall.Survival.Time.in.Days, OS.ID,Event.Free.Survival.Time.in.Days, Event.ID,CR.status.at.end.of.course.1,CR.status.at.end.of.course.2 ),
            by="TARGET.USI.1") %>%
...
```{r}
# table(MMP7.expn.0531$CR.status.at.end.of.course.1)
MMP7.0531.sub <- MMP7.expn.0531 %>%
  filter(grepl("CR|Not", CR.status.at.end.of.course.1)) %>%
  mutate(CR1=ifelse(CR.status.at.end.of.course.1 == "CR", 0, 1)) 
```
...
```{r}
set.seed(2018)
train <- bind_rows(filter(MMP7.0531.sub, CR1==1),sample_n(filter(MMP7.0531.sub, CR1==0), size = 109))
...
```{r}
# table(MMP7.0531.sub$NUP98.NSD1, MMP7.0531.sub$CR1) #really only slight relationship with the CR status. 
# table(MMP7.0531.sub$CEBPA.mutation, MMP7.0531.sub$CR1)
# table(MMP7.0531.sub$NPM.mutation, MMP7.0531.sub$CR1) #where as the two favorable mutations have 
# table(MMP7.0531.sub$NUP98.NSD1, MMP7.0531.sub$CR.status.at.end.of.course.2)[,c(1,3)]
```


```{r}
# m <- stats::glm.fit(y=unlist(MMP7.0531.sub$CR1), x=unlist(MMP7.0531.sub$Log2TPM),intercept = FALSE, family = binomial())
m <- stats::glm(CR1 ~ Log2TPM, data=train,  family = binomial())
m
summary(m)
```
...
```{r}
# MMP7.expn.0531 %>%
#   filter(MMP7.mutation=="Yes", FLT3.ITD.positive.=="Yes", NUP98.NSD1 =="Yes")

table(MMP7.expn.0531$FLT3_NUP98.NSD1, MMP7.expn.0531$FLT3_WT1)
```
...
```{r fig.height=7}
ggplot(MMP7.expn.0531, aes(x=FLT3_NUP98.NSD1_WT1, y=Log2TPM, fill=FLT3_NUP98.NSD1_WT1)) + 
...
ggplot(MMP7.expn.0531, aes(x=FLT3_WT1, y=Log2TPM, fill=FLT3_WT1)) + 
...
```{r fig.height=5, fig.width=12}
ggplot(MMP7.expn.0531, aes(x=FLT3_NUP98.NSD1, y=Log2TPM, fill=FLT3_NUP98.NSD1)) + 
  geom_boxplot(color="azure4") +
  labs(x="") +
  scale_fill_manual(values = c("saddlebrown",colors[1:4])) +
  theme_numX
...
MMP7.expn <- tpm.1031 %>%
  ...
  left_join(., select(CDE.1031, USI, 
                      FLT3.ITD,FLT3.Allelic.Ratio,NPM1,CEBPA,WT1.mutation,
                      Bone.marrow.leukemic.blast.percentage, wbc1, NUP98.NSD1_RNASeqCalls, cr1), by="USI") %>%
...
```{r}
MMP7.expn.sub <- MMP7.expn %>%
  filter(!is.na(cr1)) %>%
  mutate(CR1=ifelse(cr1 == 1, 0, 1)) %>%
  select(USI,CR1,Log2TPM)

table(MMP7.expn.sub$CR1, useNA = "always")
# MMP7.expn.sub
```
...
```{r}
pred.CR <- predict.glm(m, newdata = MMP7.expn.sub[,2:3], type="response")
```

```{r}
MMP7.expn.sub %>%
  mutate(pred.CR=pred.CR) %>%
  arrange(desc(pred.CR))
```

```{r}
proc <- roc(CR1 ~ pred.CR, data=MMP7.expn.sub)
auc(proc)
plot(proc)
```
...
```{r fig.height=5, fig.width=12}
ggplot(MMP7.expn, aes(x=FLT3_WT1, y=Log2TPM, fill=FLT3_WT1)) + 
  geom_boxplot(color="azure4") +
  labs(x="") +
  scale_fill_manual(values = c("saddlebrown",colors[1:4])) +
  theme_numX
...
```{r fig.height=5, fig.width=12}
ggplot(MMP7.expn, aes(x=FLT3_NUP98.NSD1, y=Log2TPM, fill=FLT3_NUP98.NSD1)) + 
  geom_boxplot(color="azure4") +
  labs(x="") +
  scale_fill_manual(values = c("saddlebrown",colors[1:4])) +
  theme_numX
...
#Does MMP7 Expression Correlate with Response to Sorafinib?
...
```{r}
table(MMP7.Sora$FLT3_NUP98.NSD1, MMP7.Sora$FLT3_WT1)
```

There are 3 triple positives, with NUP98-NSD1, FLT3-ITD, and WT1 mutations. 

```{r fig.height=5}
ggplot(subset(MMP7.Sora, Treatment.Arm == "Arm C"), aes(x=FLT3_NUP98.NSD1, y=Log2TPM, fill=FLT3_NUP98.NSD1)) +
  geom_boxplot(color="azure3") +
  labs(title="MMP7 Expression in NUP98.NSD1/FLT3-ITD AML \n Treated with Sorafinib (Arm C)", x="") +
  theme_numX +
...
```{r fig.height=5}
# tiff("TARGET_")
ggplot(subset(MMP7.Sora, Treatment.Arm == "Arm C"), aes(x=FLT3_NUP98.NSD1, y=Log2TPM, fill=FLT3_WT1, color=FLT3_NUP98.NSD1)) +
  geom_boxplot() +
  labs(title="MMP7 Expression in NUP98.NSD1/FLT3-ITD AML \n Treated with Sorafinib (Arm C)", x="") +
  theme_numX +
...
Rel.1031 <- KM.plots(df=MMP7.Sora,
